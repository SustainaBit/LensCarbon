 ...
 *
 * @notice Alternative implementation of carbob retirements, borrowed from MultirecipientFeeCollectModule, but recipients get divided into two groups that are processed differently.
...
contract CarbonRecipientFeeCollectModule is BaseFeeCollectModule, CarbonRetireBase {
    ...
    constructor(
        ...
        address retirementHelper
    ) BaseFeeCollectModule(hub, moduleGlobals) CarbonRetireBase(retirementHelper) {}
    ...
    function _validateAndStoreRecipients(
        ...
        if (len == 1) {
            if (initData.recipients[0].recipientType == RecipientType.CARBON) {
                ...
                if (initData.recipients[0].recipient == address(0))
            ...
            for (uint256 i = 0; i < len; ) {
                if (initData.recipients[i].recipientType == RecipientType.CARBON) {
                    ...
                    if (initData.recipients[i].recipient == address(0))
                        revert Errors.InitParamsInvalid();
                    if (initData.recipients[i].split == 0) revert InvalidRecipientSplits();
                } else revert Errors.InitParamsInvalid();
            ...
            if (recipients[0].recipientType == RecipientType.CARBON) {
                ...
                string memory beneficiaryString = string(
                    abi.encodePacked(
                        ...
                        ', by publisher: ',
                        Strings.toHexString(recipients[0].recipient)
                    )
                );
                    ...
                    if (recipients[i].recipientType == RecipientType.CARBON) {
                        ...
                        string memory beneficiaryString = string(
                            abi.encodePacked(
                                ...
                                Strings.toString(pubId),
                                ', by publisher: ',
                                Strings.toHexString(recipients[i].recipient)
                            )
                        );
                        string memory retirementMessage = string(
                            abi.encodePacked(
