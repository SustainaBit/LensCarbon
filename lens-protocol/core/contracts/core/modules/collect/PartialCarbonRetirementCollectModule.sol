...
import {CarbonRetireBase} from '../CarbonRetireBase.sol';
    ...
    ICollectModule
{
    ...
    function _performCarbonRetirement(
        ...
        if (checkRetirementSwapFeasibility(currency, poolToken, retirementAmount)) {
            string memory retiringEntityString = string(
                abi.encodePacked(
                    'Collecting Lens profile: ',
                    Strings.toString(profileId),
                    ', by collector:',
                    Strings.toHexString(uint256(uint160(collector)), 20)
