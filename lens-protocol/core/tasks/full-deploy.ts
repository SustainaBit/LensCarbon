...
import {
  ...
  IERC20Burnable__factory,
  IKlimaInfinity__factory,
  CarbonRetireBase__factory,
...
task('full-deploy', 'deploys the entire Lens Protocol').setAction(async ({}, hre) => {
  ...
  console.log('\n\t-- Deploying partialCarbonRetirementCollectModule --');
  const partialCarbonRetirementCollectModule = await deployContract(
    new PartialCarbonRetirementCollectModule__factory(deployer).deploy(lensHub.address, "0x8cE54d9625371fb2a068986d32C85De8E6e995f8", "0x8cE54d9625371fb2a068986d32C85De8E6e995f8", { nonce: deployerNonce++ })
  );
  ...
  await waitForTx(
    lensHub.whitelistCollectModule(partialCarbonRetirementCollectModule.address, true, { nonce: governanceNonce++ })
  );
  ...
  const addrs = {
    ...
    'partial carbon retirement collect module': partialCarbonRetirementCollectModule.address,
